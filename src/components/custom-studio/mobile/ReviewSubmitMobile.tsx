...
import { SHAPE_LABELS, LENGTH_LABELS, FINISH_LABELS, EFFECT_LABELS, RHINESTONE_LABELS, CHARM_LABELS, NAIL_ART_LABELS, FINGER_NAMES } from '@/lib/pricing';
...
export const ReviewSubmitMobile = ({
  ...
  const ChecklistRow = ({
    ...
    return <button onClick={onClick} className={cn("w-full flex items-center gap-3 py-4 border-b border-border text-left group overflow-hidden", !hasContent && "opacity-50")}>
        <div className={cn("w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0", hasContent ? "bg-primary/10" : "bg-muted")}>
          {hasContent ? <Check className="w-4 h-4 text-primary" /> : <Circle className="w-4 h-4 text-muted-foreground" />}
        </div>
        <span className={cn("flex-1 text-base min-w-0 truncate", hasContent ? "text-foreground" : "text-muted-foreground")}>
          {hasContent ? summary : emptyText}
        </span>
        <ChevronRight className="w-5 h-5 text-muted-foreground group-hover:text-foreground transition-colors flex-shrink-0" />
      </button>;
  };
  return <div className="w-full overflow-hidden pb-36">
      <div className="w-full pt-2 overflow-hidden">
        ...
        {/* Price Display Card */}
        <button onClick={() => setShowBreakdown(!showBreakdown)} className="w-full bg-card border border-border rounded-2xl p-6 text-center">
          ...
          {showBreakdown && <div className="mt-4 pt-4 border-t border-border text-left space-y-2">
              {priceBreakdown.items.map((item, index) => <div key={index} className="flex justify-between text-sm gap-2">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden">
        ...
        <div className="bg-background px-5 pb-8 pt-4">
          <Button onClick={hasCustomArtwork ? onRequestQuote : onAddToCart} disabled={isSubmitting} className="w-full h-14 text-lg font-semibold rounded-full shadow-lg">
            {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : hasCustomArtwork ? <>
